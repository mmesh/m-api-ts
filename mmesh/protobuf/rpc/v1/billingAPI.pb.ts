...
export class BillingAPI {
  static SearchCustomer(req: BillingCustomer.Customer, initReq?: fm.InitReq): Promise<StatusStatus.SearchResponse> {
    return fm.fetchReq<BillingCustomer.Customer, StatusStatus.SearchResponse>(`/api/v1/customers:search`, {...initReq, method: "POST", body: JSON.stringify(req, fm.replacer)})
  }
  ...
  static DeleteInvoice(req: BillingInvoice.Invoice, initReq?: fm.InitReq): Promise<StatusStatus.StatusResponse> {
    return fm.fetchReq<BillingInvoice.Invoice, StatusStatus.StatusResponse>(`/api/v1/accounts/${req["accountID"]}/billing/invoices/${req["invoiceID"]}`, {...initReq, method: "DELETE"})
  }
  ...
  static ApplyPromotion(req: BillingSubscription.ApplyPromotionRequest, initReq?: fm.InitReq): Promise<StatusStatus.StatusResponse> {
    return fm.fetchReq<BillingSubscription.ApplyPromotionRequest, StatusStatus.StatusResponse>(`/api/v1/accounts/${req["accountID"]}/subscription:promotion`, {...initReq, method: "POST", body: JSON.stringify(req, fm.replacer)})
  }
}